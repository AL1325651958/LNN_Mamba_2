...
**图1.** LNMamba架构示意图。两个带LNN动态门控的Liquid-Gated Selective SSM处理块，接收168小时ECMWF NWP序列，输出未来24小时的99分位数预测。
...
时序编码器的核心是Liquid-Gated Selective SSM块[10]。给定输入序列 $\mathbf{x} \in \mathbb{R}^{B \times L \times d}$，每个块依次执行因果卷积、选择性扫描和门控融合：
...
(\boldsymbol{\Delta}, \mathbf{B}, \mathbf{C}) &= \mathbf{W}_{\text{proj}} \mathbf{u}, \quad \boldsymbol{\Delta} \in \mathbb{R}^{B \times L \times 1},\; \mathbf{B},\mathbf{C} \in \mathbb{R}^{B \times L \times d_{\text{state}}} \\
...
其中 $\mathbf{A} \in \mathbb{R}^{d_{\text{inner}} \times d_{\text{state}}}$ 是学习得到的对角状态转移矩阵，$\boldsymbol{\Delta}$ 是输入依赖的离散化步长，$\mathbf{D} \in \mathbb{R}^{d_{\text{inner}}}$ 是学习得到的跳跃连接向量。并行关联扫描（parallel associative scan）实现了 $\mathcal{O}(L)$ 时间复杂度，同时保持了选择性机制的表达能力。

#### 2.1.2 LNN 动态门控

标准选择性状态空间块使用固定学习的参数进行状态转移，但实际风电数据表现出非平稳动态：稳定期后突然的阵风、季节性变化和日周期。我们引入LNN门控，根据当前时序上下文调制每个选择性SSM块的信息流。

门控以轻量级GRU实现，具备输入依赖的时间常数调制特性：
...
门控信号 $\boldsymbol{\alpha}^{(t)} \in (0,1)^{d}$ 在每一时间步、每一特征通道上独立计算，允许细粒度的自适应调控。GRU的隐状态更新数学上等价于连续时间ODE的欧拉离散化——其更新门 $\mathbf{z}_t$ 和重置门 $\mathbf{r}_t$ 的联合作用提供了类似液态时间常数 $\tau(\mathbf{x})$ 的输入依赖行为。当输入快速变化时（如阵风开始），更新门响应迅速，产生较高的 $\boldsymbol{\alpha}$ 值，允许更多信息流过SSM；在稳定期间，门控饱和到较低水平，抑制噪声传播。

**注：** 我们采用基于GRU的LNN门控实现，而非Hasani等人[13]的完全连续时间ODE公式。这在保持核心"液态"自适应的同时，简化了实现并降低了计算开销。我们在§6中讨论了这一设计选择的权衡。

#### 2.1.3 分位数解码器

最后一个选择性SSM块的最终隐藏状态通过两层解码器投影，同时输出所有99个分位数：
...
其中 $\hat{\mathbf{Q}} \in \mathbb{R}^{B \times 24 \times 99}$ 包含24个预测时步中每个时步和每个分位数水平 $\tau \in \{0.01, 0.02, \ldots, 0.99\}$ 的预测分位数值。
...
其中 $e_{i,h,k} = y_{i,h} - \hat{Q}_{i,h,k}$ 是分位数水平 $\tau_k$ 下的预测误差，$N$ 为批次大小，$H = 24$ 为预测时步数，$K = 99$ 为分位数数量。

### 2.4 从概率预测到点预测

为获得点预测，通过99个分位数的梯形积分计算预测分布的期望值：

$$\hat{y}_{i,h} = \sum_{k=1}^{K-1} \frac{\hat{Q}_{i,h,k} + \hat{Q}_{i,h,k+1}}{2} \cdot (\tau_{k+1} - \tau_k)$$

相比直接使用中位数 $\hat{Q}_{i,h,50}$（第50个分位数），期望值能产生更准确的点预测：风电功率分布呈显著右偏（大量时间接近零，偶发飙高），中位数系统性低估功率水平，而期望值通过积分各分位数合理捕获了高功率尾部的贡献。
...
**表2a. GEFCom2014 Zone 1 上 12 个模型的概率预测性能对比。** 所有模型在同一数据划分下评估，指标为加权 Pinball Loss（99分位数，168h输入→24h输出）。粗体为本文方法。

| 方法 | 年份/会议 | Pinball Loss | 相对LNMamba |
|------|----------|-------------|------------|
...
**表2b. GEFCom2012 风电场1上与基线方法的对比。** 所有指标在归一化功率 $[0,1]$ 上。LNMamba在 pinball loss 上比 QRF 好 19.7%，比持久性好 32.4%。
...
LNMamba 在两个数据集上均一致地超越所有对比方法。在 GEFCom2014 Zone 1 上，LNMamba（Pinball 0.2069）比最强深度基线GRU（0.2161）提升4.3%，比时序专门模型 DLinear（0.2227）提升7.1%，比最新的 iTransformer（0.2414）提升14.3%。在 GEFCom2012 Farm 1 上，LNMamba（0.0806）比 QRF 提升19.7%，比持久性提升32.4%。值得注意的是，传统机器学习方法 GRU 和 QRF 在多个数据集上均表现稳健，而多数专门设计的时序Transformer架构在该数据规模下未能展现优势。
...
| 对比 | DM统计量 | p值 | 显著时步数 |
|------|---------|-----|-----------|
| LNMamba vs Persistence | +12.432 | <0.0001 | 23/24 |
